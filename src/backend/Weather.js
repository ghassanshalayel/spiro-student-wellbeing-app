...
async function getWeatherFromCurrentLocation() {
  ...
  const speedPref = data?.settings?.speedUnit || "km/h";

  const windUnit = speedPref === "mph" ? "mph" : "kmh";
  ...
  try {
    ...
    const apiURL = `https://api.open-meteo.com/v1/forecast?latitude=${latitude}&longitude=${longitude}&hourly=temperature_2m,wind_speed_120m&temperature_unit=${tmpUnit.toLowerCase()}&wind_speed_unit=${windUnit}`;
